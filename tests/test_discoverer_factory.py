...
def test_discoverer_factory_create_bcra_series():
    ...
    assert isinstance(discoverer, ISourceDiscoverer)
...
def test_discoverer_factory_register():
    ...
    mock_client = Mock(spec=IHTTPClient)
    mock_discoverer_class = Mock(spec=ISourceDiscoverer)
    mock_discoverer_class.return_value = Mock(spec=ISourceDiscoverer)
